...
$redirectTo = htmlspecialchars($_GET['redirect'] ?? '/index.php');
$msg = $_GET['msg'] ?? '';
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title>Sign In — NFT Gallery</title>
  <meta name="description" content="Sign in to your NFT Gallery account to view and manage your NFTs.">
...
<body>
  <main class="auth-page">
    ...
    <div class="auth-card">
      <h1>Welcome back</h1>
      <p class="subtitle">Sign in to access your NFT collection</p>

      <!-- Message box -->
      <div class="auth-msg <?= $msg === 'logged_out' ? 'info show' : '' ?>" id="authMsg">
        <span id="authMsgText">
          <?= $msg === 'logged_out' ? '👋 You have been signed out.' : '' ?>
        </span>
      </div>
      ...
      <!-- Email / Password form -->
      <form id="loginForm" onsubmit="signInEmail(event)" novalidate>
        ...
        <div style="text-align:right; margin-bottom:1.25rem;">
          <a href="/auth/forgot.php"
            style="font-size:.82rem; color:var(--muted); text-decoration:none; transition:color .2s;"
            onmouseover="this.style.color='var(--accent1)'" onmouseout="this.style.color='var(--muted)'">
            Forgot password?
          </a>
        </div>
  ...
  <script type="module">
    ...
    function setLoading(btnId, loading) {
      ...
      if (loading) {
        btn.disabled = true;
        btn.innerHTML = '<span class="spinner-btn"></span> Signing in…';
      } else {
        btn.disabled = false;
    ...
    // ── Verify ID token with PHP backend ─────────
    ...
    async function handleAuthSuccess(user) {
      ...
      if (result.success) {
        ...
        showMsg('Server verification failed. Please try again.', 'error');
      }
    }

    // ── Email / Password sign-in ─────────────────
    window.signInEmail = async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        showMsg(messages[err.code] || err.message);
      }
    };
